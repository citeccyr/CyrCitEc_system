...
sub build_bundles {
  ...
  my $r_list;
  if($group) {
    $r_list=&Cec::Groups::compile_papids($group);
  }
  else {
    die "I should not be here\n";
    $r_list=retrieve $files->{'r_list'};
  }
  my $do_html=shift // '1';
  my $count_bundle=0;
  my $bundle;
  ...
  while($bundle=&Cec::Bundles::get($r_list,$count_bundle)) {
    ...
    ## some header bundle
    if(not $group and $bundle->[0] eq 'lrn@handle') {
      #print "I skip\n";
      next;
    }
    if($o{'R'}) {
    ...
    my $bunshid;
    if($group) {
      my $name=$Cec::Groups::names->[$count_bundle-1]
	// confess "I have no name for your bundle $count_bundle.";
      $bunshid="$group:$name";
    }
    else {
      $bunshid=&Cec::Bundles::id($r_list->[$count_bundle-1]);
    }
    ...
    my $out_file;
    if($group) {
      my $name=$Cec::Groups::names->[$count_bundle-1] // confess "I need this here.";
      $out_file=$Cec::Paths::dirs->{'groups'}."/$group/$name.bundle.xml";
    }
    else {
